...
(test all on desktop(google, edge, firefox, opera), iphone and sansung)
...
- All text is visible. 
- X icon is highlighted when hovered over, and it succesfully closes modal when clicked.

## Settings Modal
- Modal opens and closes as expected. 
- All text and elements are visible. 
- X icon is highlighted when hovered over, and it succesfully closes modal when clicked.
- Sound checkbox is behaving as expected. Sound is on/off corrosponding with the checkbox
...
The user's scores are stored locally to the device, so they can acess them at a later date. The storing of scores was testind as follows:
...
- Nope button warns user that their score won't be saved and gives them one last change to go back before taking the user to the home page
- Click logo to go to home page, acts the same as the 'Nope' button above
...
|then click and hold anyv hidden tile to place a flag on it|a flag is placed on the tile|As expected|Pass
...
|Return home and repect previous step until a blank tile is revealed| When a blank tile is revealed, all it's neighbouring tiles are also revealed| As expected|Pass
...
|then press 'w'|Border box should not move as it it against edge|As exppected|Pass
